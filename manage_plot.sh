...
PROJECT_DIR=/home/beams1/USR32IDC/pvWebMonitor
MANAGE=${PROJECT_DIR}/manage_plot.sh
LOGFILE=${PROJECT_DIR}/log-manage_plot.txt
PIDFILE=${PROJECT_DIR}/pid_plot.txt
#CONFIGFILE=${PROJECT_DIR}/config.xml
EXECUTABLE_SCRIPT=${PROJECT_DIR}/dataplot_32IDC.py
RETVAL=0
...
check_pid_running(){
	...
	if [ "${PID}" == "" ]; then
		...
		if [ "${RESPONSE}" == "dataplot_4IDD.p" ]; then
			RETVAL=0
		else
			# PID is not dataplot_4IDD.py
			RETVAL=1
		fi
	fi
	return $RETVAL
...
checkup(){
    ...
    #
    # */5 * * * * /home/sector4/pvWebMonitor/4idd/manage_plot.sh checkup 2>&1 > /dev/null
